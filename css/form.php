...
if ($_SERVER['REQUEST_METHOD'] == 'GET' && $_SESSION['season'] == 0) { // si on arrive sur la page via la navigation et qu'on n'a pas déjà choisi de saison, alors on débute le formulaire de choix de saison.
    ...
    if ($_POST['new_season']) { // 1. on vient de créer une nouvelle saison, on l'enregistre en bdd et on met en session l'id de saison correspondant.
        $_SESSION['season'] = createCssNewSeason($bdd, $_POST['new_season']);
        $_POST = 0;
    ...
    $_SESSION['season'] = $_POST['season']; // on enregistre l'id de saison correspondant
    ...
    if (empty($_POST['technicien_id']) || empty($_POST['day']) || empty($_POST['team']) || empty($_POST['homeOrAway']) || $_POST['buts_fcmetz'] == "" || $_POST['buts_adversaire'] == "") { // On réalise les scripts de contrôle et on affecte les variables correspondantes aux erreurs.
        ...
        setCssJournee($bdd, $_POST['technicien_id'], $_POST['day'], $_POST['team'], $_POST['homeOrAway'], $_POST['buts_fcmetz'], $_POST['buts_adversaire'], $_SESSION['season']);
        $alerte['no-alert'] = 1;
    }

...
elseif (isset($_POST['modify_pseudo']) AND isset($_POST['old_mdp']) AND isset($_POST['new_mdp_1']) AND isset($_POST['new_mdp_2'])){ // Si l'utilisateur a décidé de changer aussi son mot de passe.
	...
	elseif($nbr != 1) { // Si le mot de passe saisi ne correspond pas à celui de la BDD, on ne va pas plus loin et on renvoie avec une variable $_GET.
		header('location:auth_modifier_utilisateur.php?log=noid');
	}
	else { // Dans tous les autres cas, on modifier la base en conséquence.
		$pseudo = htmlspecialchars($_POST['modify_pseudo']); // On rend le pseudo saisi propre.
		$_SESSION['pseudo'] = $pseudo;
		$req = $bdd->prepare('UPDATE personnel_fbln SET pseudo = :pseudo, mdp = :mdp WHERE id = :id');
		$req->execute(array(
			'pseudo' => $pseudo,
			'mdp' => $_POST['new_mdp_1'],
			'id' => $_SESSION['id']
